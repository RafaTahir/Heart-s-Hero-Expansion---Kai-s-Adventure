...
- Added automated axe coverage, keyboard-only setup traversal, 44 px control assertions, a ten-route refresh matrix, reduced-motion restoration proof, and 375/768/1280 px screenshot checks.
- Added route-change heading focus for setup/map, a persistent polite recovery/reset live region, in-app scoped reset completion, and lazy loading for adventure scene modules.
- Verified the mobile restored map visually and reviewed source/bundle configuration for restricted runtime dependencies.

### Human decisions

- Core accessibility and the complete loops remained protected; no feature expansion was introduced.

### Codex work

- Hardened focus, announcements, reset behavior, responsive evidence, public-route refresh, and scene loading.
...
- `npm run lint`: pass.
- `npm run test`: pass, 5 tests.
- `npm run test:e2e`: pass, 4 browser tests including axe on every major screen.
- `npm run build`: pass; adventure screens emit as a separate lazy chunk.
- Responsive screenshots: pass at 375, 768, and 1280 px.
- Restricted dependency/configuration search: pass; no OpenAI, Supabase, backend, secret, runtime generation, or environment-variable usage in source/configuration.
...
- Production-like direct refresh checks pass locally through the Vercel-equivalent SPA fallback.

### Evidence

- Axe results, keyboard assertions, touch-size assertions, route matrix, reduced-motion check, and responsive screenshots are generated by Playwright.
